...
public class BowClass : MonoBehaviour, IWeapon
{
    ...
    public void WeaponAttack(Vector3 firePostion, Vector3 TargetPostion, string targetTag)
    {
        if (!WeaponWaitingForCooldown)
        {
            ...
            Transform clone = Instantiate(projectile, firePostion + (shootDir) + new Vector3(0, 0.5f, 0), Quaternion.Euler(0, angle, 0)) as Transform;
